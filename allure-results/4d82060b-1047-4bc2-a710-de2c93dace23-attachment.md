...
- Name: 08_Web_Select_Frames_Iframe\260_CustomDropdown.spec.ts >> Verify the dropdown selection custom
- Location: tests\08_Web_Select_Frames_Iframe\260_CustomDropdown.spec.ts:3:5
...
Call log:
  - waiting for getByRole('button', { name: 'lang-trigger' })
...
```ts
  ...
  2 | 
  3 | test('Verify the dropdown selection custom', async ({page})=>{
  4 |     await page.goto("https://app.thetestingacademy.com/playwright/tables/dropdowns");
> 5 |     await page.getByRole("button", {name: "lang-trigger"}).click();
    |                                                            ^ Error: locator.click: Test timeout of 30000ms exceeded.
  6 | 
  7 |     await page.getByRole("option", {name:"Ruby"}).click();
  8 | })
```