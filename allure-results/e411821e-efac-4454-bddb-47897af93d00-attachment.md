...
- Name: 06_Multiple_Element_\256_webtable_xpath.spec.ts >> verift the xpath of Webtable
- Location: tests\06_Multiple_Element_\256_webtable_xpath.spec.ts:3:5

# Error details

```
Error: locator.count: Unexpected token "/" while parsing css selector ".//tr". Did you mean to CSS.escape it?
```
...
```ts
  ...
  2  | 
  3  | test('verift the xpath of Webtable', async ({page}) =>{
  4  |     await page.goto("https://app.thetestingacademy.com/playwright/tables/webtable");
  5  | 
  6  |     const table= page.locator('//table[@class="companies" and @id="employees-table"]');
  7  |     const row= await table.locator('.//tr');
  8  | 
> 9  |     for(let i=0; i<await row.count(); i++){
     |                              ^ Error: locator.count: Unexpected token "/" while parsing css selector ".//tr". Did you mean to CSS.escape it?
  10 |         const rowtext= await row.nth(i).textContent();
  11 | 
  12 |         if(rowtext?.includes('Priya Kapoor')){
  13 |             const col= await row.nth(i).locator('//table[@class="companies" and @id="employees-table"]//tr/th');
  14 |             const colcount= await col.count();
  15 |             for(let j=0; j<colcount; j++){
  16 |                 const coltext= await col.nth(j).textContent();
  17 |                 if(coltext?.includes('Salary')){
  18 |                     const salary= await row.nth(i).locator(`//table[@class="companies" and @id="employees-table"]//tr/td[${j+1}]`);
  19 |                     console.log(await salary.textContent());
  20 |                 }
  21 |             }
  22 |         }
  23 | 
