...
```
Error: locator.innerText: Unexpected token "/" while parsing css selector ".//tr". Did you mean to CSS.escape it?
Call log:
  - waiting for //table[@class="companies" and @id="employees-table"] >> .//tr >> internal:has-text="Priya Kapoor"i >> nth=0 >> td[data-col="email"] >> nth=0
...
```ts
  ...
  7  |     const row= await table.locator('.//tr').filter({hasText: "Priya Kapoor"}).first();
> 8  |     const col= await row.locator('td[data-col="email"]').first().innerText();
     |                                                                  ^ Error: locator.innerText: Unexpected token "/" while parsing css selector ".//tr". Did you mean to CSS.escape it?
  9  |     console.log("Email of Priya Kapoor is: "+col);
  10 |     
  11 | });
  12 | 
```